...
Полученный массив сохраняется в модель Catalog, затем выводится в консоль:
...
- При клике открывает модалку предпросмотра.

  - В предпросмотре:

    - Название, описание, категория, изображение;
    - Кнопка “В корзину” вызывает callback, переданный презентером.

**CartView**

- Отображает список товаров в модальном окне.
- Отображает общую сумму.
- Слушает события `cart:change`.

**BuyerView**

- Управляет переходами между формами заказа (`OrderForm, ContactsForm`).
...
- `sendOrder(order)` — отправка данных заказа.

**Презентер (`main.ts`)**

Связывает все слои приложения:

- Подписывается на события `cart:change` и обновляет счётчик корзины.
- Вызывает `CatalogView.render()` после загрузки товаров.
...
- Каталог подгружается из API.
- Товары добавляются и удаляются из корзины.
- Счётчик корзины обновляется динамически.
- Модальные окна закрываются по клику вне области или на крестик.
- Реализованы формы оформления и валидация.
- После оплаты корзина очищается и показывается окно “Заказ оформлен”.